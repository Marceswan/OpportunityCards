...
export default class OpportunityCard extends NavigationMixin(LightningElement) {
    ...
    connectedCallback() {
        if (!this._initialized) {
            this._isCollapsed = this.defaultCollapsed;
    ...
    get expandCollapseIcon() {
        return this._isCollapsed ? 'utility:chevronright' : 'utility:chevrondown';
    }
    ...
    get closeDateBadgeClass() {
        let baseClass = 'slds-badge slds-m-left_x-small';
        // Closed opportunities should never show red date badge
        if (this.opportunity?.isClosed) {
            return baseClass + ' slds-badge_inverse';
        }
        // Only open opportunities can be past due (red)
        if (this.isPastDue) {
